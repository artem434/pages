...
function hidePackagesBtn() {
    ...
    window.addEventListener('scroll', function (e) {
        ...
        if (
            (scrollY > targetPosition - window.innerHeight && scrollY < targetPosition + targetHeight) ||
            (scrollY > targetPositionHero - window.innerHeight && scrollY < targetPositionHero + targetHeightHero) 
        ) {
            if (!isBtnActive) { // Додана перевірка, щоб змінити стан кнопки тільки один раз
