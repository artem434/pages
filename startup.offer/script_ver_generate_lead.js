...
$(".zoho_url").submit(function (e) {
  ...
  if ($(".error").length == 0) {
    ...
    // Додаємо webinar_date
    var webinarDate = new Date();
    var formattedDate =
      webinarDate.getFullYear() +
      "-" +
      ("0" + (webinarDate.getMonth() + 1)).slice(-2) +
      "-" +
      ("0" + webinarDate.getDate()).slice(-2);

    var data = new FormData();
    var arrayData = $(form).serializeArray();
    console.log(formattedDate);
    arrayData.push({ name: "webinar_date", value: formattedDate });

    arrayData.forEach(function (el) {
      data.append(el.name, el.value + "");
    });

    // Логування даних, які відправляються
    ...
    var get_params = activeForm.find('input[name="get_params"]').val();
    ...
    $.ajax({
      ...
      .done(function (server_response) {
        console.log("Відповідь сервера:", server_response);
          ...
          if ($(form).data("form") == "callback") {
            ...
            if (typeof fbcapiData !== "undefined") {
              sendEventToFBCAPI(
                ...
                handleServerResponse(server_response, get_params)
              );
            } else {
              handleServerResponse(server_response, get_params);
            }
    ...
    if (gotogf !== "" && gotogf !== undefined) {
      $.ajax({
        ...
        .done(function (e) {
          console.log("Відповідь від GF:", e);
        })
        .fail(function (e) {
          console.log("Помилка GF:", e);
        });
    }
    ...
    if (gr_token !== "" && gr_token !== undefined) {
      $.ajax({
        ...
        .done(function (e) {
          console.log("Відповідь від GR:", e);
        })
        .fail(function (e) {
          console.log("Помилка GR:", e);
        });
    }
  } else {
  ...
  function handleServerResponse(server_response, get_params) {
    // if (server_response.link !== false) {
    //   if (!quizComplete) {
    //     LP.CORE.showModal("#modal__quiz");
    //   } else {
    //     server_response_link =
    //       get_params == "false"
    //         ? server_response.link.split("?")[0]
    //         : server_response.link;
    //     setTimeout(function () {
