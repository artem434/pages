    ...
    return {
        ...
        execute: function () {
            jquery_1.default(function ($) {
                ...
                var $lector = $('.hero__prelector-photo .hero__photo');
                var index = -1;
                var index_name = -1;
                var $name = $('.hero__list .hero__name');
                var $hero_btn = $('.hero__btn');
                if ($(window).width() > 767) {
                    var timeValue_1 = setInterval(function (interval) {
                        index = (index + 1) % $name.length;
                        $lector.eq(index).addClass('js-active');
                    }, 1000);
                    setTimeout(function () { clearInterval(timeValue_1); $hero_btn.addClass('active-btn'), $lector.addClass('js-active'); }, 7000);
                }
                if ($(window).width() > 767) {
                    $('.section__block').addClass('in');
                }
                $(".professions__item").hover(function (e) {
                    e.preventDefault();
                    $(".professions__item ").removeClass('active');
                    $(this).addClass('active');
                });
                $(".certificate__item").hover(function (e) {
                    e.preventDefault();
                ...
                if (document.location.host == "seminars.dtkt.ua") {
                    $('.js-dk-link').attr('href', 'https://promo.dtkt.ua/7eminar_march2020');
                    $('.js-platform-banner').show();
                    $('.js-btn-hidden').show();
                }
                else {
