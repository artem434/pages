...
$(document).ready(function () {
  function toggleButtonClassOnScroll() {
    ...
    function updateButtonClass() {
      ...
      let registerPosition =
        registerSection.getBoundingClientRect().top + window.scrollY;
      let scrollPosition = window.scrollY + window.innerHeight;

      if (scrollPosition >= registerPosition) {
        btn.classList.remove("btn--fix");
      } else {
        if (window.scrollY > 0) {
    ...
    window.addEventListener("scroll", updateButtonClass, { passive: true });
