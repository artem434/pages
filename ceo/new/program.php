<section class="program" id="program">
      <div class="program__container">
        <h2 class="program__title">
          <span>Структура</span> програми
        </h2>
        <div class="program__items">
          <div class="item">
            <p class="item__number">01</p>
            <h4 class="item__title">Модель системного бізнесу та роль СЕО</h4>
            <div class="item__more"></div>
            <ul class="item__info">
            <li>Типові помилки СЕО</li>
<li>Модель системного бізнесу</li>
<li>Функціонал та компетенції ефективного СЕО</li>

            </ul>
          </div>
          <div class="item">
            <p class="item__number">02</p>
            <h4 class="item__title"> Особиста ефективність СЕО </h4>
            <div class="item__more"></div>
            <ul class="item__info">
            <li>Особиста ефективність СЕО</li>
<li>Особистий та корпоративний тайм-менеджмент</li>
<li>Робота з персональним асистентом</li>
<li>Лідерство та принципи створення сильної компанії</li>
<li>Зміна моделі поведінки CEO на різних етапах розвитку компанії</li>

            </ul>
          </div>
          <div class="item">
            <p class="item__number">03</p>
            <h4 class="item__title"> Стратегія бізнесу і бізнес-планування </h4>
            <div class="item__more"></div>
            <ul class="item__info">
            <li>Бізнес-модель та product-market fit</li>
<li>Інформаційне поле маркетингу</li>
<li>Стратегія масштабування компанії</li>
<li>Розвиток продукту компанії та NPS</li>
<li>Управління шляхом клієнта</li>
<li>Benchmarking</li>
<li>Аналіз конкурентів та конкурентна стратегія</li>

            </ul>
          </div>
          <div class="item">
            <p class="item__number">04</p>
            <h4 class="item__title"> Система залучення та втримання клієнтів </h4>
            <div class="item__more"></div>
            <ul class="item__info">
            <li>Стратегія залучення та утримання клієнтів</li>
<li>Inbound та outbound підходи у залученні клієнтів</li>
<li>Brand-маркетинг та performance-маркетинг</li>
<li>Юніт-економіка</li>
<li>Інтернет-маркетинг та його значення для залучення клієнтів</li>
<li>Управління продажами</li>

            </ul>
          </div>
          <div class="item">
            <p class="item__number">05</p>
            <h4 class="item__title"> Організаційна структура і регламентація зон відповідальності </h4>
            <div class="item__more"></div>
            <ul class="item__info">
            <li>Головні принципи побудови організаційної структури</li>
<li>C-level та ключові центри відповідальності в організаційній структурі</li>
<li>Типи організаційних структур</li>
<li>Еволюція організаційної структури з розвитком компанії</li>
<li>Регламентація зон відповідальності</li>

            </ul>
          </div>
          <div class="item">
            <p class="item__number">06</p>
            <h4 class="item__title"> Бізнес-процеси і організація ефективної взаємодії в команді </h4>
            <div class="item__more"></div>
            <ul class="item__info">
            <li>Використання бізнес-процесів</li>
<li>Формат BPMN 2.0</li>
<li>Принцип внутрішнього клієнта</li>
<li>Підвищення якості взаємодії у команді</li>

            </ul>
          </div>
          <div class="item">
            <p class="item__number">07</p>
            <h4 class="item__title"> HR-менеджмент і розвиток корпоративної культури </h4>
            <div class="item__more"></div>
            <ul class="item__info">
            <li>Роль HR-функції у компанії</li>
<li>Управління якісним складом команди</li>
<li>Ефективний процес рекрутингу</li>
<li>Канали пошуку кандидатів на вакансії</li>
<li>Система вступної адаптації та атестації</li>

            </ul>
          </div>
          <div class="item">
            <p class="item__number">08</p>
            <h4 class="item__title"> Мотивація команди, розвиток співробітників та керівників </h4>
            <div class="item__more"></div>
            <ul class="item__info">
            <li>Управління мотивацією співробітників</li>
<li>Управління залученістю співробітників до роботи</li>
<li>Принципи розробки систем фінансової мотивації</li>
<li>Оцінка та розвиток персоналу</li>
<li>Корпоративна культура та принципи її формування</li>
<li>Розвиток керівників</li>

            </ul>
          </div>
          <div class="item">
            <p class="item__number">09</p>
            <h4 class="item__title"> Бренд-маркетинг </h4>
            <div class="item__more"></div>
            <ul class="item__info">
            <li>Управління впізнаваністю бренду та лояльністю</li>
<li>Медіа-тиск та share of voice</li>
<li>ATL- та BTL-реклама</li>
<li>PR</li>
<li>SMM для розвитку бренду</li>
<li>Контент-маркетинг</li>
<li>Influence-маркетинг</li>
<li>Event-маркетинг</li>

            </ul>
          </div>
          <div class="item">
            <p class="item__number">10</p>
            <h4 class="item__title"> Performance-маркетинг і управління продажами </h4>
            <div class="item__more"></div>
            <ul class="item__info">
            <li>Логіка performance-маркетингу</li>
<li>Математика performance-маркетингу</li>
<li>Управління психологією цільового клієнта</li>
<li>Inbound та Outbound підходи до залучення клієнтів</li>
<li>Воронка залучення клієнтів</li>
<li>Функція продажів та лідогенерації</li>
<li>Відділ продажів в організаційній структурі компанії</li>
<li>Робота з CRM</li>
<li>Технології та стандарти продажів</li>

            </ul>
          </div>
          <div class="item">
            <p class="item__number">11</p>
            <h4 class="item__title"> Управління фінансами </h4>
            <div class="item__more"></div>
            <ul class="item__info">
            <li>Основні помилки в управлінні фінансами</li>
<li>Роль CFO та процес управління фінансами</li>
<li>Ключові фінансові звіти, балансовий звіт та власний капітал</li>
<li>Звіт про прибутки та збитки (P&L) та управління прибутковістю бізнесу</li>
<li>Звіт про рух грошових коштів (Cashflow) та управління ліквідністю бізнесу</li>
<li>Фінансова модель</li>
<li>Планування та бюджетування</li>
<li>Додаткові матеріали: "Логіка відображення операцій в управлінській звітності"</li>

            </ul>
          </div>
          <div class="item">
            <p class="item__number">12</p>
            <h4 class="item__title"> Операційне управління </h4>
            <div class="item__more"></div>
            <ul class="item__info">
            <li>Регулярний менеджмент</li>
<li>Правила ефективного проведення управлінських заходів</li>
<li>Побудова dashboard-ів та система внутрішньої звітності</li>
<li>Підходи до управління проектами</li>
<li>Забезпечення дисципліни виконання завдань</li>

            </ul>
          </div>
          <div class="item">
            <p class="item__number">13</p>
            <h4 class="item__title"> Стратегічне планування і забезпечення дисципліни реалізації стратегії </h4>
            <div class="item__more"></div>
            <ul class="item__info">
            <li>Ритм заходів щодо стратегічного планування</li>
<li>Бачення майбутнього компанії та CEO view</li>
<li>Підготовка і проведення стратегічної сесії</li>

            </ul>
          </div>
          <div class="item">
            <p class="item__number">14</p>
            <h4 class="item__title">Залучення інвестицій  </h4>
            <div class="item__more"></div>
            <ul class="item__info">
            <li>Залучення інвестицій</li>
<li>Методи оцінки вартості бізнесу</li>
<li>Раунди інвестицій</li>
<li>Джерела пошуку інвесторів</li>
<li>Правила побудови відносин з інвесторами</li>
<li>Продаж бізнесу</li>

            </ul>
          </div>
          <div class="item">
            <p class="item__number">15</p>
            <h4 class="item__title">Міжнародний розвиток <span>[пакет участі PREMIUM]</span></h4>
            <div class="item__more"></div>
            <ul class="item__info">
            <li>Бізнес-модель міжнародного розвитку</li>
<li>Вибір географії та макроекономічний аналіз країн</li>
<li>Побудова прямих міжнародних продажів віддалено</li>
<li>Продажі через дилерів та дистриб'юторів</li>
<li>Відкриття власної філії в іншій країні</li>
<li>Організація продажів через маркетплейси</li>
<li>Продаж міжнародної франшизи</li>

            </ul>
          </div>
          <div class="item">
            <p class="item__number">16</p>
            <h4 class="item__title">HR-маркетинг і бренд роботодавця <span> <br>[пакет участі PREMIUM]</span></h4>
            <div class="item__more"></div>
            <ul class="item__info">
            <li>Бренд роботодавця та управління HR-продуктом</li>
<li>Точки контакту в HR-маркетингу</li>
<li>Робота з відгуками про роботу в компанії</li>
<li>Звільнення співробітників та exit-інтерв'ю</li>

            </ul>
          </div>
          <div class="item">
            <p class="item__number">17</p>
            <h4 class="item__title">Управління проектами <span>[пакет участі PREMIUM]</span></h4>
            <div class="item__more"></div>
            <ul class="item__info">
            <li>Проектний менеджмент та етапи реалізації проектів</li>
<li>Очікувані результати роботи менеджера проекту</li>
<li>Каскадний підхід до управління проектами (діаграма Ганта)</li>
<li>Agile-підхід до управління проектами</li>
<li>Автоматизація управління проектами</li>

            </ul>
          </div>
          <div class="item">
            <p class="item__number">18</p>
            <h4 class="item__title"> <span>Бонусний модуль від Дмитра Стрижова</span> <br>Роль корпоративного управління та Advisory Board у масштабуванні бізнесу <span>[пакет участі PREMIUM]</span></h4>
            <div class="item__more"></div>
            <ul class="item__info">
            <li>Етапи розвитку бізнесу в Україні</li>
<li>"Чарівна пігулка" для бізнесу – Advisory Board</li>
<li>Роль Board на різних етапах життєвого циклу компанії</li>
<li>Переваги залучення фахівців до Advisory Board</li>
<li>Створення Advisory Board: практичний досвід холдингу "Шериф"</li>
<li>Кращі практики для ефективної роботи Board</li>
<li>Рекомендації для створення Advisory Board</li>

            </ul>
          </div>
        </div>
        <div class="block">
          <div class="block__content">
            <h2 class="block__title">Авторська методологія <b>та система навчання</b>
            </h2>
            <p class="block__text">дозволять у найкоротший термін отримати практичні інструменти й алгоритми з побудови системного управління компанією та опанувати навички топ-управлінця.</p>
          </div>
          <div class="block__buttons">
            <p class="block__buttons_text">Залиш заявку, щоб забронювати участь.</p>
            <a class="block__buttons_btn" href="#register">ЗАРЕЄСТРУВАТИСЯ</a>
          </div>
        </div>
      </div>
    </section>