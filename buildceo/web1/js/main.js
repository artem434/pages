...
const nextWednesday = new Date(
  dateCopy.setDate(dateCopy.getDate() + ((7 - dateCopy.getDay() + 3) % 7 || 7))
);
...
let meet_date = "22 листопада";
$(".cDate").text(meet_date);
/*
let today_date = new Date();
function getNextWednesday(currentDate) {
  const dayOfWeek = currentDate.getDay();
  if (dayOfWeek === 3) { // Проверяем, среда ли сегодня
    currentDate.setDate(currentDate.getDate() + 14); // Добавляем 14 дней, если сегодня среда
  } else {
    let daysUntilWednesday = (3 + 7 - dayOfWeek) % 7; // Вычисляем сколько дней до следующей среды
    currentDate.setDate(currentDate.getDate() + daysUntilWednesday + 7); // Прибавляем эти дни плюс 7 дней следующей недели
  }
  return currentDate;
}
var lastChangeDate = $.cookie('lastChangeDate');
// Перевірка, чи потрібно змінити дату    
var currentDate = new Date();
var currentDay = currentDate.getDay(); // Отримуємо поточний день тижня (неділя - 0, понеділок - 1, ..., субота - 6)    
var currentHour = currentDate.getHours(); // Отримуємо поточну годину (від 0 до 23)
// Перевірка, чи сьогодні вівторок і чи є поточний час після 19:00
console.log('lastChangeDate', lastChangeDate);
if(lastChangeDate != '') lastChangeDate = currentDate.toISOString();
if (currentDay === 3 && currentHour >= 19) {        // Якщо дата останньої зміни не встановлена або пройшло більше 2 тижнів з останньої зміни
  if (!lastChangeDate || (currentDate - new Date(lastChangeDate)) >= 14 * 24 * 60 * 60 * 1000) {            // Додаємо 2 тижні до поточної дати
    currentDate.setDate(currentDate.getDate() + 14);
    // Зберігаємо нову дату в куках на 2 тижнів
    $.cookie('lastChangeDate', currentDate.toISOString(), { expires: 14, path: '/' });
    // Отримуємо нову дату та місяць у форматі DD/MM
    console.log('currentDate', currentDate);
    var newDay = currentDate.getDate();
    var newMonth = currentDate.getMonth() + 1; // Місяці у JavaScript починаються з 0 (січень) до 11 (грудень)
    // Оновлюємо текст з новою датою та місяцем у форматі DD/MM
  }
}
console.log('today_date', today_date);
console.log('today_date.getDate()', today_date.getDate());
console.log('today_date.getMonth()', today_date.getMonth());
if (today_date.getDate() > 8 && today_date.getMonth() >= 10) {
  console.log('today_date.getMonth()', today_date.getMonth());
  $('.cDate').text(('0' + newDay).slice(-2) + '/' + ('0' + newMonth).slice(-2));
}
*/
