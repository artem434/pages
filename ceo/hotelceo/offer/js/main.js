...
const packagePro = document.querySelector(
  ".packages-btns__pack-name.pro_price"
);
const packagePremium = document.querySelector(
  ".packages-btns__pack-name:not(.pro_price)"
);

function activatePackage(selectedPackage) {
  document.querySelectorAll(".packages-btns__pack-name").forEach((item) => {
    item.classList.remove("tabs__tab--active");
  });
  selectedPackage.classList.add("tabs__tab--active");
}

btnPro.addEventListener("click", (event) => {
  event.preventDefault();
  activatePackage(packagePro);
});

btnPremium.addEventListener("click", (event) => {
  event.preventDefault();
  activatePackage(packagePremium);
});

document.querySelectorAll(".packages-btns__pack-name").forEach((item) => {
  item.addEventListener("click", () => {
    document
      .querySelectorAll(".packages-btns__pack-name")
      .forEach((item) => item.classList.remove("tabs__tab--active"));
