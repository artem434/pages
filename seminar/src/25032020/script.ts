...
jQuery(($) => {
    ...
    const $lector = $('.hero__prelector-photo .hero__photo');
    let index = -1;
    let index_name = -1;
    const $name = $('.hero__list .hero__name');
    const $hero_btn = $('.hero__btn');

    if ($(window).width() > 767) {
        let timeValue = setInterval((interval) => {
            index = (index + 1) % $name.length;
            $lector.eq(index).addClass('js-active');

        }, 1000);
        setTimeout(() => { clearInterval(timeValue); $hero_btn.addClass('active-btn'),  $lector.addClass('js-active'); }, 7000);
    }
