...
if($('.ticker__swiper').length) {
    const bcTicker = new Swiper('.ticker__swiper', {
		loop: true,
        speed: 5000,
        slidesPerView: 'auto',
		spaceBetween: 18,
...
$('.program .desc__title').on('click', function() {
	let targetRow = $(this).closest('.program__row');
...
$('.program .desc__title').first().click();
...
var waypoints = $('#packages, #register').waypoint({
	handler: function(direction) {
	  $('.hero__btn').addClass('hidden');
	}
  })
  
  var waypoints = $('#hero, #facts, #description, #program, #speaker, #skills, #about, #for, #company, #clients, #footer').waypoint({
	handler: function(direction) {
